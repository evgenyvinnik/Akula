...
public class initializer {
	...
	public bootstrap_DB get_solo_matrix()
	{
		bootstrap_DB DB = new bootstrap_DB(10, 0);
		for(int i = 1; i < 10; i++)
		{
			DB.input_data(i, null, (double)length[i]);
		}
		return DB;
	}
	
	public bootstrap_DB get_ipc_matrix()
	{
		bootstrap_DB DB = new bootstrap_DB(10, 0);
		for(int i = 1; i < 10; i++)
		{
			DB.input_data(i, null, IPC[i]);
		}
		return DB;
	}
	
	public bootstrap_DB get_mpi_matrix()
	{
		bootstrap_DB DB = new bootstrap_DB(10, 0);
		for(int i = 1; i < 10; i++)
		{
			DB.input_data(i, null, MPI[i]);
		}
		return DB;
	}
